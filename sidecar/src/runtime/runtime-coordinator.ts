...
export class RuntimeCoordinator {
  ...
  start(parameters: ConversationStartParams, notify: (event: AgentEvent) => void): ConversationStartResult {
    if (!parameters.conversationId.trim() || !parameters.input.trim()) throw new Error("Conversation and input are required.");
    ...
    void adapter.run({ ...parameters, runId }, emit, cancellation.signal)
      ...
      .finally(() => {
        ...
        if (current?.runId === runId) this.activeRuns.delete(parameters.conversationId);
      });
