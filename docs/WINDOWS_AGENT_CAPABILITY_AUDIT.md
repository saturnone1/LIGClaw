...
| 화면·OCR·음성 | 부분 | ADR 0033 개인정보 경계, composer 사용자 동작 기반 Windows picker 단일 프레임, D3D11/WARP, preview crop·로컬 OCR·민감 buffer zeroing, 기본 꺼짐 누르고 말하기 STT·마이크 manifest·PCM/중간 결과 비저장, 사용자가 직접 선택한 답변의 Windows 로컬 TTS·즉시 중지 | 설치 실행과 Windows 10/11 OCR/STT/TTS 실기 matrix, 자동 읽기·방해 금지 opt-in |
