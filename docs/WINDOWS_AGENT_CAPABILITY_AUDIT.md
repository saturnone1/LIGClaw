...
| 화면·OCR·음성 | 부분 | ADR 0033 개인정보 경계, composer 사용자 동작 기반 Windows picker 단일 프레임, D3D11/WARP, preview 드래그 crop·OCR 재실행·전체 복원, package identity 기반 로컬 OCR, 4K 비율 축소, `PreparedSensitiveContext` 상한·identity·전달 뒤에도 유지되는 2분 만료·zeroing, 선택 마스킹·취소 기본값·확인 텍스트만 합류, 기본 꺼짐의 누르고 말하기 STT·마이크 manifest·PCM/중간 결과 비저장 | 설치 실행과 Windows 10/11 OCR/STT 실기 matrix, 응답별 TTS |
