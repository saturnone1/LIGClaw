...
다음 실제 구현 진입점은 C-5 Explorer 선택 텍스트/우클릭 진입점이다. 현재 실행 중인 사용자 앱이 종료되면 picker·crop·STT·TTS·자동 읽기 설정의 UI smoke를 재검증하고, Windows SDK가 있는 환경에서 unsigned MSIX와 release manifest를 생성한다. C-3 Windows 10 실기기·production 서명·실앱 UIA와 설치 상태 OCR/STT/TTS gate는 환경이 준비되는 대로 병행한다.
