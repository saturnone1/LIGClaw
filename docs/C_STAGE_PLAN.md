...
상태: 진행 중. 1차 슬라이스에서 schema 1~11 migration, 단일 connection/gate, pending restore와 backup 검증을 `ConversationDatabase` 단독 소유로 옮겼다. 미래 schema 거부 후 파일 잠금 해제를 회귀 테스트로 추가했고 저장소 경계는 ADR 0026에 기록했다. 2차 슬라이스에서 conversation run/event/search/transcript SQL을 `IConversationRepository`/`ConversationRepository`로 옮기고 composition root와 직접 replay 테스트가 facade 대신 adapter를 사용하게 했다. 다음은 감사·승인·undo repository를 분리한다.
