...
**상태: 진행 중 (2026-08-02).** 상세 순서와 완료 조건은 `docs/C_STAGE_PLAN.md`를 따른다. Windows PowerShell 5.1·PowerShell 7 검증 기준선, Sidecar Tool catalog, Desktop 대화 orchestration, SQLite repository와 설정 section 책임 분리를 완료했다. bounded release evidence 자동화를 추가했고 외부 Windows 10 22H2·서명 설치·실앱 UIA gate는 별도 추적한다. C-4는 Protocol 1.14 R0 전원, Protocol 1.15 R1 bounded 상위 앱 리소스, Protocol 1.16 R1 현재 네트워크 상세, Protocol 1.17 R0 비식별 장치 상태 진단까지 구현했다. C-5는 ADR 0033과 Desktop 메모리 전용 `PreparedSensitiveContext`의 상한·identity 결합·전달 뒤에도 유지되는 2분 만료·zeroing 수명, 이미지/OCR 실제 내용 preview·선택 마스킹·취소 기본값·OCR-only export에 더해, composer 사용자 동작→Windows 시스템 picker→bounded 단일 프레임→선택 영역 crop·재인식→package identity 기반 로컬 OCR→확인한 텍스트만 입력 합류하는 수직 경로를 구현했다. 설정 기본 꺼짐의 누르고 말하기 STT와 사용자가 transcript에서 고른 답변 글을 Windows 로컬 음성으로 읽고 즉시 중지하는 TTS도 구현했다. 다음은 자동 읽기·방해 금지 시간 opt-in과 설치 실행·Windows 10/11 실기 acceptance다.
