...
**상태: 진행 중 (2026-07-25).** 단일 인스턴스, 기존 창 복원, 트레이 상주, 사용자 선택형 빠른 호출, 시작프로그램, 진행/오류/취소 UI와 범용 OpenAI 호환 `Base URL/API Key/Model` 연결 및 Windows Credential Manager 저장을 완료했다. 설정 항목은 서로 독립적으로 저장되며 연결 작업의 중복 실행 방지, HTTP 보안 안내, 고배율 스크롤을 제공한다. 다음 수직 기능은 conversation/event persistence이다.
