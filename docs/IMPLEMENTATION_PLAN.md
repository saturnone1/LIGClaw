...
**상태: 진행 중 (2026-08-01).** 상세 순서와 완료 조건은 `docs/C_STAGE_PLAN.md`를 따른다. Windows PowerShell 5.1·PowerShell 7 검증 기준선과 Sidecar Tool catalog 분리를 완료했으며, 다음은 Desktop 대화 orchestration, SQLite repository, 설정 section을 동작 보존형 수직 슬라이스로 분리한다. 로컬 출시 자동화와 외부 Windows 10 22H2·서명 설치·실앱 UIA gate를 분리해 추적하고, 다음 사용자 기능은 이 경계가 안정된 뒤 전원·프로세스·네트워크·장치 진단 순으로 추가한다.
