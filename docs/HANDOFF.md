...
- C단계 Protocol 1.15 Windows 10/11 공통 느린 PC 진단, R1 매회 승인 뒤 PID·경로·사용자명 없이 상위 CPU·메모리 앱 그룹 최대 10개
- C단계 Protocol 1.16 현재 IP/prefix·DNS·게이트웨이·연결 SSID 진단, R1 매회 승인과 최신 Windows 위치 권한 실패 격리
- C단계 Protocol 1.17 장치명·ID 없이 기본 오디오 출력, 활성 디스플레이·주 화면 해상도, 프린터 구성·기본 프린터 오프라인 상태를 제공하는 R0 진단
- C-5 ADR 0033 사용자 주도 화면 picker, 실제 내용 preview·마스킹, 메모리 전용 준비 수명, package identity와 STT/TTS opt-in 경계
- C-5 `PreparedSensitiveContext` 4,096px/16MP/8MiB 이미지·32KiB OCR 상한, conversation/run/tool-call 결합, 2분 자동 만료와 교체·거부·완료·종료 zeroing 기반
- C-5 화면 원문 전용 preview 창, 이미지·OCR 동시 검토, OCR 선택 마스킹·전체 삭제, 취소 기본값, 잘못된 이미지·32KiB 초과 차단과 OCR-only export
- C-5 composer `화면 가져오기`→Windows 시스템 picker→D3D11 단일 프레임 PNG→Windows 로컬 OCR→preview 확인 텍스트 합류. 10초/4,096px/16MP/8MiB 경계, hardware→WARP fallback, OCR 비율 축소와 Unicode-safe 32KiB 제한, 모든 경로 buffer zeroing 적용
- C-5 preview 드래그 영역을 원본 픽셀로 안전하게 매핑해 PNG crop·OCR 재실행, 성공 시 이미지·텍스트 원자적 교체, 실패 시 기존 내용 유지, 전체 화면 복원. store에서 preview로 전달된 뒤에도 자체 2분 만료 타이머가 원문 zeroing과 창 닫기를 보장
- C-5 설정 기본 꺼짐의 `누르고 말하기` STT, MSIX microphone capability, mouse·Space/Enter press/release와 60초 timeout, package identity·권한·언어·네트워크 오류 분리, Unicode-safe 16,000자 제한. PCM·중간 결과 로그 없이 최종 텍스트만 composer에 합류하고 사용자가 보낼 때만 일반 요청으로 처리
- C-5 transcript에서 사용자가 선택한 답변 글만 Windows 로컬 음성으로 읽는 TTS. 20,000자 경계, 합성 준비 중·재생 중 같은 버튼 즉시 중지, 완료·실패·창 종료 buffer 폐기
- C-5 기본 꺼짐 자동 읽기와 기본 22:00–07:00 방해 금지. 활성 앱·완성 응답·STT/TTS 유휴 조건에서만 재생하며 30분 단위 시작/종료 설정, 자정 교차 시간과 동일 시간 거부 정책을 테스트
- C-5 ADR 0034 Explorer activation 기반. `--explorer-item` 기존 경로 최대 20개·JSON 32KiB, 중복 제거·순서 유지, current-user Named Pipe 단일 인스턴스 전달, 파일 내용 없이 composer preview. native `IExplorerCommand`·MSIX 등록은 C++/SDK 환경 gate
- C-5 ADR 0035 기본 꺼짐 로컬 반복 작업 제안. 최근 45일 완료 요청 최대 200개를 exact-normalized 비교해 서로 다른 날짜 3회와 일간/주간 간격이 분명한 경우만 카드로 제안한다. 민감·첨부·명시적 예약 요청 제외, fingerprint·최근 제안·무시 목록 최대 100개와 14일 cooldown, 자동 실행·예약 없이 Agent 작업 편집기 prefill
